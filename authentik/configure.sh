...
if [[ -n "$LATEST_TAG" ]]; then
  # Check if AUTHENTIK_TAG already exists in .env
  EXISTING_AUTHENTIK_TAG=$(grep -E '^AUTHENTIK_TAG=' "$ENV_FILE" 2>/dev/null | head -n1 | cut -d= -f2- || true)

  echo ""
  if [[ -n "$CURRENT_TAG" ]]; then
    if [[ "$CURRENT_TAG" == "$LATEST_TAG" ]]; then
      echo "OK: Your currently used Authentik tag (${CURRENT_TAG}) matches the latest release."
      # If AUTHENTIK_TAG is missing, we can still offer to set it explicitly
      if [[ -z "$EXISTING_AUTHENTIK_TAG" ]]; then
        read -rp "AUTHENTIK_TAG is not set in .env. Set it to '${LATEST_TAG}' now? [y/N]: " SET_TAG
        SET_TAG=${SET_TAG:-N}
        if [[ "$SET_TAG" =~ ^[Yy]$ ]]; then
          sed -i '/^AUTHENTIK_TAG=/d' "$ENV_FILE"
          echo "AUTHENTIK_TAG=${LATEST_TAG}" >> "$ENV_FILE"
